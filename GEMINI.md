...
2. **Type Safety**: Strictly utilize the shared `@planitpoker/shared` package for all Socket events, payloads, and domain models. Avoid using `any` or redefining types locally.
3. **Component Architecture**: Keep React components "dumb". Extract complex logic into custom hooks. Ensure styling is modular and does not pollute the JSX.
4. **Real-time Communication**: Never put core business logic inside socket event listeners. Keep the transport layer decoupled from the domain logic.
5. **Accessibility**: All UI components must be fully accessible (WAI-ARIA compliant, keyboard navigable).
